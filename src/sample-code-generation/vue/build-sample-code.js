...
module.exports = function(sampleCodeMetadata, prettierConfig = {}, propsInfo = {}, knobs) {
  ...
  // not using self closing tags in vue templates (https://github.com/vuejs/vue/issues/1036)
  const openTagPostfix = '>';
  const closeTag = `</${sampleCodeMetadata.componentName}>`;

  // prettier was supposed to handle this, but in case where sourceTemplate has some surrounding tags around INJECTED_SOURCE_PLACEHOLDER
  // it doesn't remove extra spaces (only in vue template, in react it works)
  const trimmedPropsString = trimEnd(propsString);
  const storySubject = `${openTag}${trimmedPropsString}${openTagPostfix}${childrenString}${closeTag}`;
  const sampleCode = sampleCodeMetadata.sourceTemplate.replace(INJECTED_SOURCE_PLACEHOLDER, storySubject);
  const prettierOptions = Object.assign({}, { parser: 'vue', plugins }, prettierConfig);
...
function getSinglePropString(prop, propDocgenInfo, knobs) {
  // If a value wasn't set for the prop we treat it as value-less attribute (ie v-slot)
